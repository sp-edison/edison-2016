		...
		<table width="100%" border="0" cellspacing="0" cellpadding="0">
			<thead>
				<tr> 
					...
					<th width="25%"><liferay-ui:message key="edison-table-list-header-virtuallab" /></th>
					<th width="25%"><liferay-ui:message key="edison-virtuallab-tablerow-virtualclass" /></th>
					<th width="12%"><liferay-ui:message key="edison-virtuallab-virtualLabClassManagement-student-number" /></th>
				</tr> 
			</thead>
	...
	if(dataList != null){
		...
		if(dataList.length > 0){
			for(var a=0; a<dataList.length; a++){
				...
				if(dataList[a].classTitle == "Total User"){
					$userTableTr = $("<tr/>").css("background-color", "#E3E3E3");
					$("<td/>").addClass("TR").attr("colspan", "4").html(dataList[a].affiliation+" 합계").appendTo($userTableTr);
					$("<td/>").addClass("TR").html(dataList[a].user_count).appendTo($userTableTr);	
					
					totalCnt += dataList[a].user_count;
				}else{
					$userTableTr = $("<tr/>");
					$("<td/>").addClass("TC").html(++rownum).appendTo($userTableTr);
					$("<td/>").addClass("TC").html(dataList[a].affiliation).appendTo($userTableTr);
					$("<td/>").addClass("TC").html(dataList[a].virtualLabTitle).appendTo($userTableTr);
					$("<td/>").addClass("TC").html(dataList[a].classTitle).appendTo($userTableTr);
					$("<td/>").addClass("TR").html(dataList[a].user_count).appendTo($userTableTr);
				}
...
// 		$("#totalSpan").html("<b>"+rownum+" <liferay-ui:message key='edison-search-article'/></b>");
		...
		$("<tr/>").append(
							$("<td/>").attr("colspan","5").html('<liferay-ui:message key="edison-there-are-no-data" />')
	...
	if(pieChartOrganigationList != null){
		...
		if(pieChartOrganigationList.length > 0){
			for(var c=0; c<pieChartOrganigationList.length; c++){			
				...
				if($("#<portlet:namespace/>jobUniversityField").val() != ""){
					title += " / " + pieChartOrganigationList[c].virtualLabTitle;
					title += " / " + pieChartOrganigationList[c].classTitle;
				}			
				var cnt = pieChartOrganigationList[c].user_count;	
