...
function generateInputFile( inputdeckUserForm )
{
	...
	for(var i=0; i<variableNames.length;i++)
	{
		...
		if( useVariable )
		{
			if( variable.getType() == InputdeckConstants.VARIABLE_TYPE_NUMERIC && $('#'+portletNameSpace+name+"_chcekd").is(':checked')=='true')
			{
				isSweep = true;
				sweepName = name;
				sweepStep = value;
	...
	if( isSweep )
	{
		...
		while( index <= count && sweepValue <= sweepMax)
		{
			...
			cloneUserForm.getVariable(sweepName).clearSweep();
...
function getListFormHtml( id, variable )
{
	...
	inputHtml +='</select></td><td>' +variable.getLocalizedDescription( defaultLanguage )+'</td></tr>';
	return inputHtml;
}
