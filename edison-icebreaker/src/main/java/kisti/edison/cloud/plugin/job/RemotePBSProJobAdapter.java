...
@Component("PBSPro")
public class RemotePBSProJobAdapter implements JobAdapter {
...
//			s = "#PBS -o " + job.getWorkingDir() + "/" + job.getUuid() + ".out";
	...
	public Job submit(User user, Cluster cluster, Job job) {
		...
		RemotePBSProJob submittedJob = null;
		...
		if (job.getType() instanceof Job.JobType) {
			...
			LOG.info("PBSPro makeJobScriptFile");
		...
		RemotePBSProJob j = new RemotePBSProJob(job.getUuid(), scriptPath);
		try {
			...
			if (jobId != null && !jobId.isEmpty()) {
				...
				submittedJob = RemotePBSProJob.getJobById(cluster, job, jobId);
			} else {
	...
	public Job getInformation(User user, Cluster cluster, Job job) {
		...
		RemotePBSProJob j = null;

		try {
			j = RemotePBSProJob.getJobById(cluster, job, job.getJobId());
...
//		
//		RemotePBSProJobAdapter pbspro = new RemotePBSProJobAdapter();
//		
